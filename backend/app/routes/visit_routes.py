...
from app.models.visit_model import Visit, VisitStatus
...
# ── Seller: get visits for their lands ───────────────────────────────────────
...
def my_land_visits(
    ...
    seller_land_ids = [l.id for l in db.query(Land).filter(Land.seller_id == current_user.id).all()]
    if not seller_land_ids:
        return []
    visits = (db.query(Visit)
                .filter(Visit.land_id.in_(seller_land_ids))
